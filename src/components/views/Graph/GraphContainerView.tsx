...
export class GraphContainerView extends React.Component<GraphContainerViewProps, GraphContainerViewState> {
    ...
    changeSubView(viewType: ViewType) {
        this.setState({
            viewType: viewType
        })
    }
    ...
    render() {
        return  <SplitPane className='scrollable hide-resizer' split="horizontal"  defaultSize={40} allowResize={false} >
                    <div className='h-100 w-100 toolbar'>
                        ...
                        &nbsp;
                        &nbsp;
                        <button className={'btn btn-sm' + (this.state.viewType === ViewType.Inheritance ? ' btn-info' : ' btn-outline-info')}
                            onClick={() => this.changeSubView(ViewType.Inheritance)}>
                            Inheritance
                        </button>
                        &nbsp;
                        <button className={'btn btn-sm' + (this.state.viewType === ViewType.TypeResolution ? ' btn-info' : ' btn-outline-info')}
