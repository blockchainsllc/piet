...
export class ContractEventView extends React.Component<ContractEventViewProps, {}> {
    ...
    componentDidMount() {
        this.setState({
            eventCollapsed: Array(this.props.selectedContract.events.length + this.props.selectedContract.inheritedEvents.length).fill(false)
        })
    }

    componentWillReceiveProps(nextProps: ContractEventViewProps) {
    ...
    getEventList (contract: Sol.Contract, inherited: boolean) {
        const events = inherited ? contract.inheritedEvents : contract.events
        const eventIndexOffset = inherited ? contract.events.length : 0
            ...
            return  <div className='member-parent-container selected-list-item list-group-item list-group-item-action flex-column align-items-start with-detailed-view'
                ...
                <div className='member-container'>
                    <div className='left-member'>
                        <button data-target={'#event' + contract.name + event.name} data-toggle='collapse'
                        type='button' onClick={() => this.toogleCollapse(eventIndexOffset + eventIndex)} 
