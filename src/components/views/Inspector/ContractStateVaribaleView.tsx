...
export class ContractStateVaribaleView extends React.Component<ContractStateVaribaleViewProps, {}> {
    ...
    onStateVariableInputChange(e, stateVariableName: string) {
        e.persist()
        this.setState((prevState: ContractStateVaribaleViewState) => {
            ...
            const stateVariables = [...this.props.selectedContract.stateVariables, ...this.props.selectedContract.inheritedStateVariables]
            const stateVariableIndex = stateVariables.findIndex((sv: Sol.ContractStateVariable) => sv.name === stateVariableName)
    ...
    componentDidMount() {
        const numberOfStateVariables = this.props.selectedContract.stateVariables.length
    ...
    componentWillReceiveProps(nextProps: ContractStateVaribaleViewProps) {
    ...
    async call(stateVariable: Sol.ContractStateVariable, index: number) {
        ...
        if (stateVariable.visibility === 'public' 
            ...
            const abi = getStateVariableAbi(stateVariable, this.props.web3, this.props.contracts)
            ...
            let result 
            try {
                const arrayOrMappingKey = stateVariable.solidityType.mapping || stateVariable.solidityType.isArray 
    ...
    getStateVariableList (contract: Sol.Contract, inherited: boolean) {
        
        const stateVariables = inherited ? contract.inheritedStateVariables : contract.stateVariables
        ...
        return stateVariables.map((stateVariable: Sol.ContractStateVariable, index: number) => {
            const svIndexOffset = inherited ? contract.stateVariables.length : 0
