...
export class ContractModifierView extends React.Component<ContractModifierViewProps, {}> {
    ...
    componentDidMount() {
        this.setState({
            modifierCollapsed: Array(this.props.selectedContract.modifiers.length + this.props.selectedContract.inheritedModifiers.length).fill(false)
        })
    }

    componentWillReceiveProps(nextProps: ContractModifierViewProps) {
    ...
    getModifierList(contract: Sol.Contract, inherited: boolean) {
        const modifiers = inherited ? contract.inheritedModifiers : contract.modifiers
        const modifierIndexOffset = inherited ? contract.modifiers.length : 0
            ...
            return  <div className='member-parent-container selected-list-item list-group-item list-group-item-action flex-column align-items-start with-detailed-view'
                        ...
                        <div className='member-container'>
                            <div className='left-member'>
                                <button data-target={'#modifier' + contract.name + modifier.name} data-toggle='collapse'
                                type='button' onClick={() => this.toogleCollapse(modifierIndexOffset + modifierIndex)} className='btn btn-outline-dark detailed-button left-member'>
